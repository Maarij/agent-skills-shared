...
function Ensure-Junction {
    ...
    if (-not (Test-Path -LiteralPath $Source)) {
        throw "Missing source skill for ${Name}: $Source"
    }

    if (-not (Test-Path -LiteralPath (Split-Path -Parent $Destination))) {
        if ($DryRun) {
            Write-Output "Would create destination root $(Split-Path -Parent $Destination)"
        }
        else {
...
foreach ($skillName in Get-ManagedSkills $repoRootPath) {
    Ensure-Junction `
        ...
        -Destination (Join-Path $destinationRootPath $skillName)
}
